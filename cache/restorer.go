...
func (r restorer) Restore(dsts []string) error {
    ...
    if len(dsts) == 0 {
        prefix := strings.TrimPrefix(filepath.Join(namespace, key), "/")
        if !strings.HasSuffix(prefix, "/") && r.enableCacheKeySeparator {
            prefix = prefix + "/"
        }
    ...
    for _, dst := range dsts {
        src := strings.TrimPrefix(filepath.ToSlash(filepath.Join(namespace, key, dst)), "/")
...
func (r restorer) restore(src, dst string) (err error) {
    ...
    go func() {
        ...
        // Ensure src path uses forward slashes and has no leading slash
