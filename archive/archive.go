...
func FromFormat(logger log.Logger, root string, format string, opts ...Option) Archive {
	...
	switch format {
	case Gzip:
		return gzip.New(logger, root, options.skipSymlinks, options.preserveMetadata, options.compressionLevel)
	case Zstd:
		return zstd.New(logger, root, options.skipSymlinks, options.preserveMetadata, options.compressionLevel)
	case Tar:
